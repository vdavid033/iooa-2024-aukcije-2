...
export default {
    ...
    async mounted() {
        ...
        await this.dohvatiKorisnika(id);
        this.ispisiPodatke();
    },
    ...
    methods: {
        async dohvatiKorisnika(id){
          try {
            const response = await axios.get("http://localhost:3000/api/korisnikinfo/" + id);
        ...
        async izmjenaKorisnika(){
            ...
            if(this.korisnik_novo.prezime_korisnika=='')
                this.korisnik_novo.prezime_korisnika = this.korisnik_trenutno.prezime_korisnika;
            if(this.korisnik_novo.email_korisnika=='')
                this.korisnik_novo.email_korisnika = this.korisnik_trenutno.email_korisnika;
            if(this.korisnik_novo.adresa_korisnika=='')
                this.korisnik_novo.adresa_korisnika = this.korisnik_trenutno.adresa_korisnika;
            
            //console.log("Uspjeh!");
            try {
               const response = await axios.put("http://localhost:3000/api/izmjenakorisnika/", this.korisnik_novo);
