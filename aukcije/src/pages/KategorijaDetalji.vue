<template>
 <q-page style="margin-left:2%; margin-right:2%" window-height window-width>
        ...
        <q-form>
            ...
            <p ref="p_naziv"></p>
            <q-btn type="submit" label="Izmijeni" color="primary" class="q-mt-md" @click="izmjenaKategorije()"/>
        </q-form> 
    </q-page>
</template>
...
export default {
    ...
    async mounted() {
        ...
        await this.dohvatiKategoriju(id);
        this.ispisiPodatke();
    },

    methods: {
        async dohvatiKategoriju(id){
          try {
            const response = await axios.get("http://localhost:3000/api/kategorijainfo/" + id);
        ...
        async izmjenaKategorije(){
            ...
            try {
               const response = await axios.put("http://localhost:3000/api/izmjenaKategorije", this.kategorija_novo);

               this.$q.notify({
                color: 'positive',
                position: 'top',
                message: 'Izmjena podataka uspješna!' });
                await this.dohvatiKategoriju(this.kategorija_novo.id_kategorije);
